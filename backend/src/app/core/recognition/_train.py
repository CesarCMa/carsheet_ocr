...
def train(opt, show_number=2, amp=False):
    ...
    model = VGG_Model(opt)
    logger.info(
        f"model input parameters {opt.imgH} {opt.imgW} {opt.num_fiducial} {opt.input_channel} {opt.output_channel} {opt.hidden_size} {opt.num_class} {opt.batch_max_length} {opt.Transformation} {opt.FeatureExtraction} {opt.SequenceModeling} {opt.Prediction}"
    )
