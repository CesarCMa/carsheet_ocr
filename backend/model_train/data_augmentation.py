...
@click.option(
    ...
    required=True,
    help="Directory containing images and labels.csv.",
)
@click.option(
    "--output-dir",
    type=click.Path(file_okay=False, dir_okay=True),
    required=True,
    help="Directory to save augmented images and new labels.",
)
@click.option(
    "--num-augmentations",
    type=int,
...
def augment_data(input_dir, output_dir, num_augmentations):
    ...
    transform = A.Compose(
        [
            A.Rotate(
                limit=15, p=0.5, border_mode=cv2.BORDER_CONSTANT, value=0
            ),  # Gentle rotation
    ...
    with tqdm(total=len(df) * num_augmentations, desc="Augmenting Images") as pbar:
        for index, row in df.iterrows():
            ...
            try:
                ...
                click.echo(
                    f"Warning: Image file not found: {image_path}. Skipping.", err=True
                )
                continue
            except Exception as e:
                click.echo(
                    f"Warning: Error loading image {image_path}: {e}. Skipping.",
                    err=True,
                )
                continue
            ...
            new_labels_data.append(
                {"filename": original_output_filename, "words": words}
            )
            ...
            for i in range(num_augmentations):
                try:
                    ...
                    click.echo(
                        f"Warning: Error augmenting image {filename} (iteration {i}): {e}. Skipping augmentation.",
                        err=True,
                    )
                    pbar.update(
                        1
                    )  # Still update progress bar even if augmentation fails
