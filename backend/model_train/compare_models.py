...
def parse_args():
    ...
    parser.add_argument('--tuned_model', required=True, help='Path to tuned model (.pth)')
...
def load_training_options(tuned_model_path):
    ...
    if not os.path.exists(opt_path):
        raise FileNotFoundError(f"opt.txt not found in {os.path.dirname(tuned_model_path)}")
...
def load_model(model_path, train_settings, device):
    ...
    converter = CTCLabelConverter(
        character=train_settings["character"], 
        lang_list=train_settings["lang_list"]
    )
...
def evaluate_model(model, evaluation_loader, converter, train_settings, device):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        preds_str = [pred.replace('[', '').replace(']', '').replace("'", '') for pred in preds_str]
...
def main():
    ...
    train_settings = {
        ...
        'character': train_options.get('character', '0123456789abcdefghijklmnopqrstuvwxyz'),
        ...
        'network_params': train_options.get('network_params', {
            'input_channel': 1,
            'output_channel': 256,
            'hidden_size': 256
        }),
    ...
    base_results = evaluate_model(base_model, valid_loader, base_converter, train_settings, device)
    
    print("\nEvaluating Tuned Model...")
    tuned_results = evaluate_model(tuned_model, valid_loader, tuned_converter, train_settings, device)
    ...
    for (gt, base_pred), (_, tuned_pred) in zip(base_results['predictions'], tuned_results['predictions']):
